...
async function main() {
    ...
    const seasons = [];
    for (const farm of farms) seasons.push(await prisma.cropSeason.upsert({ where: { farmId_year_sequence: { farmId: farm.id, year: 2026, sequence: 1 } }, update: { name: "Niên vụ 2025-2026", status: "ACTIVE" }, create: { farmId: farm.id, name: "Niên vụ 2025-2026", year: 2026, sequence: 1, status: "ACTIVE", startedAt: at("2025-08-01") } }));
    ...
    const coopmart = await prisma.distributionDestination.upsert({ where: { name_address: { name: "Hệ thống Siêu thị Co.opmart", address: "TP.HCM" } }, update: {}, create: { name: "Hệ thống Siêu thị Co.opmart", type: "RETAIL", province: "TP.HCM", address: "TP.HCM" } });
    const winmart = await prisma.distributionDestination.upsert({ where: { name_address: { name: "Chuỗi Siêu thị WinMart", address: "Đồng Nai" } }, update: {}, create: { name: "Chuỗi Siêu thị WinMart", type: "RETAIL", province: "Đồng Nai", address: "Đồng Nai" } });
    const hocmon = await prisma.distributionDestination.upsert({ where: { name_address: { name: "Chợ đầu mối Hóc Môn", address: "Hóc Môn, TP.HCM" } }, update: {}, create: { name: "Chợ đầu mối Hóc Môn", type: "MARKET", province: "TP.HCM", address: "Hóc Môn, TP.HCM" } });

    const scenarios = [
        { 
            lotCode: "TP-20260827-001", 
            token: "TV-PROCESS-DEMO", 
            owner: processor, 
        ...
        { 
            ...
            token: "TV-COLLECTOR-RETAIL-DEMO", 
            owner: collector, 
        ...
        { 
            lotCode: "CM-RETAIL-001", 
            token: "TV-RETAIL-DEMO", 
            owner: collector, 
            ownerUser: collectors[0], 
            ownerType: "COLLECTOR" as const, 
            sourceType: "COLLECTION_LOT" as const, 
            ...
            stockBeforeDispatch: 800,
            buyerName: "Chợ đầu mối Thủ Đức",
            unitPrice: 80000,
            subtotal: 24000000,
            discount: 0,
        ...
        { 
            lotCode: "CM-EXPORT-001", 
            token: "TV-EXPORT-DEMO", 
            owner: processor, 
            ownerUser: processors[0], 
            ownerType: "PROCESSING_FACILITY" as const, 
            sourceType: "FINISHED_PRODUCT_LOT" as const, 
            ...
            destination: china, 
            quantity: 200, 
            productName: "Sầu riêng Ri6 cấp đông xuất khẩu", 
            stockBeforeDispatch: 1500,
            buyerName: "China Import Corp",
            unitPrice: 160000,
            subtotal: 32000000,
            discount: 0,
        ...
        { 
            lotCode: "CM-SUSPENDED-001", 
            token: "TV-SUSPENDED-DEMO", 
            owner: collector, 
            ownerUser: collectors[0], 
            ownerType: "COLLECTOR" as const, 
            sourceType: "COLLECTION_LOT" as const, 
            ...
            quantity: 50, 
            productName: "Sầu riêng tươi Dona", 
            stockBeforeDispatch: 100,
            buyerName: "Đại lý Nông sản",
            unitPrice: 75000,
            subtotal: 3750000,
            discount: 0,
            totalAmount: 3750000,
        ...
        { 
            lotCode: "CM-REVOKED-001", 
            token: "TV-REVOKED-DEMO", 
            owner: processor, 
            ownerUser: processors[0], 
            ownerType: "PROCESSING_FACILITY" as const, 
            sourceType: "FINISHED_PRODUCT_LOT" as const, 
            ...
            stockBeforeDispatch: 100,
            buyerName: "Siêu thị TriViet Demo",
            unitPrice: 140000,
            subtotal: 7000000,
            discount: 0,
            totalAmount: 7000000,
    ...
    for (const scenario of scenarios) {
        const commercial = await prisma.commercialLot.upsert({ 
            where: { lotCode: scenario.lotCode }, 
            update: {
                stockBeforeDispatch: scenario.stockBeforeDispatch,
            ...
            create: { 
                lotCode: scenario.lotCode, 
                ownerType: scenario.ownerType, 
                ownerId: scenario.owner.id, 
                sourceType: scenario.sourceType, 
                sourceId: scenario.sourceId, 
                sourceCollectionLotId: scenario.sourceCollectionLotId, 
                sourceFinishedProductLotId: scenario.sourceFinishedProductLotId, 
        ...
        if (scenario.paidAmount > 0) {
            await prisma.partnerPaymentRecord.deleteMany({ where: { commercialLotId: commercial.id } });
            await prisma.partnerPaymentRecord.create({
                data: {
                    facilityId: scenario.owner.id,
                    commercialLotId: commercial.id,
                    type: "RECEIPT",
                    amount: scenario.paidAmount,
                    paymentDate: at("2026-08-24"),
                    paymentMethod: scenario.paymentMethod,
                    payerName: scenario.buyerName,
                    note: `Thanh toán đợt 1 cho lô ${scenario.lotCode}`,
                },
            });
        }
        ...
        await prisma.traceEvent.createMany({ data: [
            ...
            { commercialLotId: commercial.id, entityType: "COMMERCIAL_LOT", entityId: commercial.id, eventType: "COMMERCIAL_LOT_CREATED", eventTime: at("2026-08-23"), title: "Lô xuất bán được tạo", description: `${scenario.lotCode} · Bên mua: ${scenario.buyerName}`, isPublic: true },
    ...
    // Seed sample operating expenses for Collector and Processor
    await prisma.partnerExpense.deleteMany({ where: { facilityId: { in: [collector.id, processor.id] } } });
    await prisma.partnerExpense.createMany({
        data: [
            // June expenses
            {
                facilityId: processor.id,
                category: "PROCESSING_LABOR",
                title: "Nhân công chế biến sầu riêng tháng 6",
                amount: 8000000,
                paidAmount: 8000000,
                status: "PAID",
                expenseDate: at("2026-06-16"),
                paymentMethod: "Chuyển khoản",
                recipient: "Tổ nhân công Trị An",
            },
            {
                facilityId: processor.id,
                category: "COLD_STORAGE_ELECTRICITY",
                title: "Tiền điện kho lạnh bảo quản tháng 6",
                amount: 5500000,
                paidAmount: 5500000,
                status: "PAID",
                expenseDate: at("2026-06-25"),
                paymentMethod: "Chuyển khoản",
                recipient: "Điện lực Đồng Nai",
            },
            {
                facilityId: collector.id,
                category: "LOGISTICS_TRANSPORT",
                title: "Thuê xe tải vận chuyển sầu tháng 6",
                amount: 3500000,
                paidAmount: 3500000,
                status: "PAID",
                expenseDate: at("2026-06-18"),
                paymentMethod: "Chuyển khoản",
                recipient: "Đội xe tải Thành Công",
            },
            // July expenses
            {
                facilityId: processor.id,
                category: "PACKAGING",
                title: "Khay nhựa định hình & thùng carton tháng 7",
                amount: 8500000,
                paidAmount: 8500000,
                status: "PAID",
                expenseDate: at("2026-07-11"),
                paymentMethod: "Chuyển khoản",
            ...
            },
            {
                facilityId: processor.id,
                category: "PROCESSING_LABOR",
                title: "Nhân công bóc tách múi sầu riêng tháng 7",
                amount: 14000000,
                paidAmount: 14000000,
                status: "PAID",
                expenseDate: at("2026-07-12"),
                paymentMethod: "Chuyển khoản",
                recipient: "Tổ nhân công Trị An",
            },
            {
                facilityId: processor.id,
                category: "COLD_STORAGE_ELECTRICITY",
                title: "Tiền điện kho lạnh bảo quản tháng 7",
                amount: 7200000,
                paidAmount: 7200000,
                status: "PAID",
                expenseDate: at("2026-07-25"),
                paymentMethod: "Chuyển khoản",
                recipient: "Điện lực Đồng Nai",
            },
            {
                facilityId: collector.id,
                category: "LOGISTICS_TRANSPORT",
                title: "Thuê xe tải vận chuyển hàng tháng 7",
                amount: 4000000,
                paidAmount: 4000000,
                status: "PAID",
                expenseDate: at("2026-07-15"),
                paymentMethod: "Chuyển khoản",
                recipient: "Đội xe tải Thành Công",
            },
            // August expenses
            {
                facilityId: processor.id,
                category: "PROCESSING_LABOR",
                title: "Nhân công bóc tách múi ca sáng & chiều",
                amount: 12000000,
                paidAmount: 12000000,
                status: "PAID",
                expenseDate: at("2026-08-23"),
                paymentMethod: "Chuyển khoản",
                recipient: "Tổ nhân công Trị An",
                note: "Ca tách múi sầu riêng cấp đông 650kg",
            },
            {
                facilityId: processor.id,
                category: "PACKAGING",
                title: "Bao bì hút chân không & khay nhựa cấp đông",
                amount: 6500000,
                paidAmount: 6500000,
                status: "PAID",
                expenseDate: at("2026-08-23"),
                paymentMethod: "Chuyển khoản",
                recipient: "Công ty Bao bì Xanh",
                note: "1.300 khay & màng hút chân không",
            },
            {
                facilityId: processor.id,
                category: "COLD_STORAGE_ELECTRICITY",
                title: "Tiền điện kho lạnh bảo quản tháng 8",
                amount: 8500000,
                paidAmount: 4000000,
                status: "PARTIAL",
                expenseDate: at("2026-08-24"),
                paymentMethod: "Chuyển khoản",
                recipient: "Điện lực Đồng Nai",
                note: "Kho bảo quản -18 độ C",
            },
            {
                facilityId: collector.id,
                category: "LOGISTICS_TRANSPORT",
                title: "Thuê xe tải vận chuyển sầu từ vườn về vựa",
                amount: 4500000,
                paidAmount: 4500000,
                status: "PAID",
                expenseDate: at("2026-08-20"),
                paymentMethod: "Chuyển khoản",
                recipient: "Đội xe tải Thành Công",
                note: "Vận chuyển 4 chuyến vườn Long Khánh & Tân Phú",
            },
            {
                facilityId: collector.id,
                category: "PROCESSING_LABOR",
                title: "Tiền công bốc xếp và phân loại sầu riêng tại vựa",
                amount: 3200000,
                paidAmount: 3200000,
                status: "PAID",
                expenseDate: at("2026-08-21"),
                paymentMethod: "Tiền mặt",
                recipient: "Tổ bốc xếp vựa Thành Phát",
            },
        ],
    });
    ...
    const directSeason = await prisma.cropSeason.upsert({ where: { farmId_year_sequence: { farmId: directFarm.id, year: 2026, sequence: 1 } }, update: { name: "Niên vụ 2025-2026", status: "ACTIVE" }, create: { farmId: directFarm.id, name: "Niên vụ 2025-2026", year: 2026, sequence: 1, status: "ACTIVE", startedAt: at("2025-08-01") } });
