...
export async function observationData(request: Request, farmerId: string, existingObservedAt?: Date) {
    ...
    const timeOfDay = String(form.get("timeOfDay") || "Cả ngày");
    ...
    if ([temperatureMax, temperatureMin, humidity, soilHumidity, rainfallMm, windSpeed].some(Number.isNaN)) {
        throw new Error("Các chỉ số đo phải là số hợp lệ.");
    }
    if (temperatureMax != null && temperatureMin != null && temperatureMin > temperatureMax) {
        throw new Error("Nhiệt độ thấp nhất không được lớn hơn nhiệt độ cao nhất.");
    }
    if ((humidity != null && (humidity < 0 || humidity > 100)) || (soilHumidity != null && (soilHumidity < 0 || soilHumidity > 100))) {
        throw new Error("Độ ẩm phải nằm trong khoảng 0–100%.");
    }
    ...
    if (files.some(file => !file.type.startsWith("image/") || file.size > 5 * 1024 * 1024)) {
        throw new Error("Mỗi ảnh phải đúng định dạng và không vượt quá 5 MB.");
    }
    ...
    return {
        ...
        timeOfDay,
        condition,
        temperature: temperatureMax ?? temperatureMin ?? null,
        temperatureMax,
        temperatureMin,
        humidity,
        soilCondition,
