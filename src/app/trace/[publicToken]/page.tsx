    ...
    return <main className="min-h-screen overflow-x-hidden bg-gradient-to-b from-emerald-50 to-white px-4 py-6 text-slate-900 sm:py-10">
        <div className="mx-auto max-w-5xl space-y-5">
            <header className="rounded-3xl border border-emerald-100 bg-white p-5 shadow-sm sm:p-8">
                ...
                <dl className="mt-6 grid grid-cols-2 gap-4 border-t pt-6 text-sm lg:grid-cols-3"><Info label="Mã truy xuất" value={trace.code} /><Info label="Đơn vị phát hành" value={`${trace.issuerType === "FARMER" ? "Hộ sản xuất" : trace.issuerType === "COLLECTOR" ? "Vựa thu mua" : "Cơ sở chế biến"}: ${trace.issuer}`} /><Info label="Điểm đến" value={trace.destination?.name || "Chưa xác định"} /></dl>
            </header>
