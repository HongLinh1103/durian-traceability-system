...
import { ROLE_LABELS } from "@/lib/account-permissions";
import { DEFAULT_ROLE_PERMISSIONS, getAllSystemPermissionKeys } from "@/lib/permissions-data";
...
export async function GET() {
    try {
        ...
        return NextResponse.json({
            success: true,
            data: users.map((user) => {
                const defaultPerms = DEFAULT_ROLE_PERMISSIONS[user.role]?.permissions || [];
                ...
                return {
                    ...
                    permissions: savedPermissions ?? defaultPerms,
...
export async function PUT(request: Request) {
  try {
    ...
    const user = await prisma.user.findFirst({
        where: { id: userId, role: { not: "ADMIN" }, deletedAt: null },
    ...
    const permissions = [...new Set(requested)].filter((key) => validKeys.has(key));
    const adminName = session.user.fullName || session.user.phone || "Admin";
    
    await prisma.$transaction([
        prisma.userPermissionConfig.upsert({
            where: { userId },
            update: { permissions, updatedById: session.user.id, updatedByName: adminName },
...
export async function DELETE(request: Request) {
  try {
    ...
    return NextResponse.json({ success: true, message: "Đã khôi phục quyền mặc định theo vai trò thành công." });
  } catch (error) {
