...
const schema = z.object({ areaManagerId: z.string().min(1), growingRegionId: z.string().min(1), note: z.string().trim().max(500).optional() });
...
export async function POST(request: Request) {
    ...
    await prisma.$transaction(async tx => {
        await tx.areaManagerRegionAssignment.updateMany({ where: { growingRegionId: region.id, isActive: true }, data: { isActive: false, endedAt: new Date(), note: "Được thay thế bởi phân công mới" } });
        await tx.areaManagerRegionAssignment.create({ data: { ...parsed.data, assignedById: session.user.id } });
    });
    return NextResponse.json({ success: true, message: "Đã phân công Trưởng ban vùng trồng." });
}
