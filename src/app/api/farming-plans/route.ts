...
export async function POST(request: Request) {
    ...
    const plan = await prisma.farmingPlan.create({ data: { farmerId: session.user.id, farmId: farm.id, plannedDate: new Date(`${parsed.data.plannedDate}T${parsed.data.plannedTime || "00:00"}:00+07:00`), title: parsed.data.title, stage: toPrismaGrowthStage(parsed.data.stage), activityType: toPrismaActivityType(parsed.data.activityType), otherActivity: parsed.data.otherActivity || null, notes: parsed.data.notes || null } });
