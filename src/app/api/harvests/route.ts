...
const schema = z.object({
    ...
    expectedWeight: z.coerce.number().positive(), weightUnit: z.enum(["kg", "tấn"]).default("kg"), fruitCondition: z.string().optional(),
...
export async function GET() {
    ...
    const data = await prisma.harvestRecord.findMany({ where: buyer ? { buyerUserId: session.user.id } : { farmerId: session.user.id }, include: { farm: { select: { farmName: true, farmCode: true, address: true, durianVariety: true } }, farmer: { select: { fullName: true, phone: true } }, buyerFacility: { select: { name: true, phone: true, province: true } } }, orderBy: { createdAt: "desc" } });
...
export async function POST(request: Request) {
    ...
    const waiting = Boolean(facility); const created = await prisma.harvestRecord.create({ data: { code, farmId: farm.id, farmerId: session.user.id, buyerType: value.buyerType, buyerFacilityId: facility?.id, buyerUserId: facility?.ownerId, status: waiting ? "WAITING_CONFIRMATION" : "DRAFT", expectedHarvestDate: new Date(value.expectedHarvestDate), expectedWeight: value.expectedWeight, weightUnit: value.weightUnit, expectedSaleWeight: facility ? value.expectedWeight : null, expectedPricePerKg: value.expectedPricePerKg, expectedBuyerArrivalDate: value.expectedBuyerArrivalDate ? new Date(value.expectedBuyerArrivalDate) : null, deliveryMethod: value.deliveryMethod, transactionNote: value.transactionNote || null, histories: { create: { actorId: session.user.id, toStatus: waiting ? "WAITING_CONFIRMATION" : "DRAFT", note: "Nông dân tạo phiếu thu hoạch" } } } });
    if (facility) await prisma.notification.create({ data: { userId: facility.ownerId, type: "HARVEST_REQUEST", title: "Phiếu thu hoạch mới", message: `${farm.farmName} gửi phiếu ${code}, dự kiến ${value.expectedWeight} ${value.weightUnit}.` } });
