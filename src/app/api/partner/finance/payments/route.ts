...
const paymentSchema = z.object({
    ...
    if (data.type === "PAYMENT" && !data.expenseId) return false;
    return true;
}, { message: "Phải chọn mã phiếu xuất bán hoặc khoản chi tương ứng" });
    ...
    try {
        const result = await prisma.$transaction(async (tx) => {
            if (value.type === "RECEIPT" && value.commercialLotId) {
                ...
                return payment;
            } else if (value.type === "PAYMENT" && value.expenseId) {
