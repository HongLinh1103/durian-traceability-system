...
export default async function FarmerJournalPage({ searchParams }: { searchParams: { tab?: string; farmId?: string; year?: string } }) {
    ...
    return <div className="mx-auto w-full max-w-[1800px] space-y-5 px-3 py-5 sm:px-4 [&>main]:min-h-0 [&>main]:max-w-none [&>main]:px-0 [&>main]:py-0 [&>main>header]:!items-center [&>main>header]:!rounded-[28px] [&>main>header]:!border [&>main>header]:!border-emerald-100 [&>main>header]:!bg-none [&>main>header]:!bg-white [&>main>header]:!p-5 [&>main>header]:!text-slate-900 [&>main>header>div]:relative [&>main>header>div]:pl-20 [&>main>header>div]:before:absolute [&>main>header>div]:before:left-0 [&>main>header>div]:before:top-1/2 [&>main>header>div]:before:grid [&>main>header>div]:before:h-16 [&>main>header>div]:before:w-16 [&>main>header>div]:before:-translate-y-1/2 [&>main>header>div]:before:place-items-center [&>main>header>div]:before:rounded-2xl [&>main>header>div]:before:bg-emerald-50 [&>main>header>div]:before:text-2xl [&>main>header>div]:before:content-['☁'] [&>main>header>div>p:first-child]:hidden [&>main>header>div>h1]:!mt-0 [&>main>header>div>h1]:!text-2xl [&>main>header>div>h1]:!text-slate-900 [&>main>header>div>p:last-child]:!mt-1 [&>main>header>div>p:last-child]:!text-sm [&>main>header>div>p:last-child]:!text-slate-500 [&>main>header>button]:!min-h-12 [&>main>header>button]:!rounded-2xl [&>main>header>button]:!bg-emerald-600 [&>main>header>button]:!px-6 [&>main>header>button]:!text-white hover:[&>main>header>button]:!bg-emerald-700">
        <nav className="grid grid-cols-3 gap-2 rounded-3xl border bg-white p-2 shadow-sm" aria-label="Loại nhật ký">
            {tabs.map(({value,label,icon:Icon})=><Link key={value} href={`/dashboard/farmer/journal?tab=${value}`} className={`flex min-h-12 items-center justify-center gap-2 rounded-2xl px-3 py-3 text-center text-sm font-bold transition ${active===value?"bg-emerald-600 text-white shadow-sm":"text-slate-600 hover:bg-emerald-50 hover:text-emerald-700"}`}><Icon className="h-5 w-5"/><span>{label}</span></Link>)}
        </nav>
