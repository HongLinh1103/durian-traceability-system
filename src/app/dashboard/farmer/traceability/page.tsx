...
export default async function Page() {
    ...
    return <main className="mx-auto max-w-7xl space-y-5 px-4 py-7 sm:px-6"><header><p className="text-sm font-bold uppercase tracking-wide text-emerald-700">Nông sản bán trực tiếp</p><h1 className="mt-1 text-3xl font-black">Lô thương mại & QR</h1><p className="mt-2 text-slate-500">Chia một phần lô thu hoạch để bán trực tiếp, chọn điểm đến và phát hành QR trước khi đưa hàng ra thị trường.</p></header><TraceabilityManager initialLots={JSON.parse(JSON.stringify(lots))} sources={sources} destinations={destinations}/></main>;
}
