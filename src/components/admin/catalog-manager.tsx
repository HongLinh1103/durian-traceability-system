...
export function CatalogManager() {
    ...
    return <main className="mx-auto w-full max-w-[1600px] space-y-5 px-4 py-7 sm:px-6">
        ...
        <nav className="grid grid-cols-3 gap-1 rounded-2xl border bg-white p-1.5 shadow-sm sm:gap-2 sm:rounded-3xl sm:p-2" aria-label="Loại danh mục">
            <button onClick={() => setTab("varieties")} className={`flex min-h-14 min-w-0 flex-col items-center justify-center gap-1 rounded-xl px-1 py-2 text-center text-sm font-bold leading-tight sm:min-h-12 sm:flex-row sm:gap-2 sm:rounded-2xl sm:px-3 sm:py-3 sm:text-base ${tab === "varieties" ? "bg-brand-600 text-white shadow-soft" : "text-slate-600 hover:bg-brand-50"}`}><Sprout className="h-4 w-4 shrink-0 sm:h-5 sm:w-5"/><span className="whitespace-nowrap">Cây giống</span></button>
            <button onClick={() => setTab("cultivation")} className={`flex min-h-14 min-w-0 flex-col items-center justify-center gap-1 rounded-xl px-1 py-2 text-center text-sm font-bold leading-tight sm:min-h-12 sm:flex-row sm:gap-2 sm:rounded-2xl sm:px-3 sm:py-3 sm:text-base ${tab === "cultivation" ? "bg-brand-600 text-white shadow-soft" : "text-slate-600 hover:bg-brand-50"}`}><Layers3 className="h-4 w-4 shrink-0 sm:h-5 sm:w-5"/><span><span className="whitespace-nowrap">Giai đoạn</span><span className="hidden sm:inline"> – </span><br className="sm:hidden"/><span className="whitespace-nowrap">Công việc</span></span></button>
            <Link href="/dashboard/admin/master-data/pesticides" className="flex min-h-14 min-w-0 flex-col items-center justify-center gap-1 rounded-xl px-1 py-2 text-center text-sm font-bold leading-tight text-slate-600 hover:bg-emerald-50 sm:min-h-12 sm:flex-row sm:gap-2 sm:rounded-2xl sm:px-3 sm:py-3 sm:text-base"><ShieldAlert className="h-4 w-4 shrink-0 sm:h-5 sm:w-5"/><span className="whitespace-nowrap">Danh mục cấm</span></Link>
        </nav>
        {tab === "varieties" ? <section className="overflow-hidden rounded-2xl border border-slate-300 bg-white shadow-sm">
            <div className="flex flex-col gap-3 border-b border-slate-200 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="relative w-full max-w-lg"><Search className="absolute left-4 top-3.5 h-5 w-5 text-slate-400"/><Input className="h-12 pl-11 rounded-xl" value={search} onChange={event => setSearch(event.target.value)} placeholder="Tìm theo tên giống..."/></div><Button className="h-12 rounded-xl" onClick={() => setVarietyForm("new")}><Plus className="mr-2 h-4 w-4"/>Thêm giống</Button></div>
            <div className="overflow-x-auto"><table className="w-full min-w-[800px] border-collapse border border-slate-300 text-left text-sm"><thead className="bg-slate-100/90 text-xs text-slate-700"><tr><th className="border border-slate-300 px-3.5 py-3 font-semibold align-middle">Tên giống</th><th className="border border-slate-300 px-3.5 py-3 font-semibold align-middle">Tên khác</th><th className="border border-slate-300 px-3.5 py-3 font-semibold align-middle">Mô tả ngắn</th><th className="border border-slate-300 px-3.5 py-3 font-semibold text-center align-middle">Trạng thái</th><th className="border border-slate-300 px-3.5 py-3 font-semibold text-center align-middle">Thao tác</th></tr></thead><tbody>{visibleVarieties.map(item => <tr key={item.id} className="hover:bg-slate-50/70 transition"><td className="border border-slate-200 px-3.5 py-2.5 font-bold text-slate-900">{item.name}</td><td className="border border-slate-200 px-3.5 py-2.5 text-slate-600">{item.alternativeName || "—"}</td><td className="max-w-md border border-slate-200 px-3.5 py-2.5 text-slate-600">{item.description || "—"}</td><td className="border border-slate-200 px-3.5 py-2.5 text-center"><span className={`rounded-full px-3 py-1 text-xs font-bold ${item.isActive ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{item.isActive ? "Đang dùng" : "Đã ẩn"}</span></td><td className="border border-slate-200 px-3.5 py-2.5 text-center"><div className="flex items-center justify-center gap-2"><Button size="sm" variant="ghost" onClick={() => setVarietyForm(item)}><Edit3 className="mr-1 h-4 w-4"/>Sửa</Button><Button size="sm" variant="ghost" onClick={() => void toggleVariety(item)}>{item.isActive ? <EyeOff className="mr-1 h-4 w-4"/> : <Eye className="mr-1 h-4 w-4"/>}{item.isActive ? "Ẩn" : "Dùng lại"}</Button></div></td></tr>)}{!visibleVarieties.length && <tr><td colSpan={5} className="border border-slate-200 p-12 text-center text-slate-500">Chưa có giống sầu riêng nào phù hợp.</td></tr>}</tbody></table></div>
