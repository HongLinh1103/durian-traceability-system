...
export default function ExportWordButton({ title, headers, rows, filename }: { title: string; headers: string[]; rows: (string | number)[][]; filename: string }) {
    ...
    return <div><button type="button" disabled={busy || !rows.length} onClick={download} className="rounded-xl bg-emerald-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{busy ? "Đang xuất..." : "Xuất file Word"}</button>{error && <p role="alert" className="text-sm text-red-700">{error}</p>}</div>;
}
