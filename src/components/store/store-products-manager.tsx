                ...
                return <article key={product.id} className="flex min-w-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                    ...
                    <div className="flex flex-1 flex-col p-3">
                        ...
                        <div className="mt-auto grid grid-cols-[1fr_auto] gap-2 pt-3">
                            {product.stock === 0 && <Button asChild size="sm"><Link href={`/dashboard/store/inventory?tab=create&action=restock&productId=${encodeURIComponent(product.id)}#warehouse-document-form`}><ArrowDownToLine className="mr-1 h-4 w-4" />{product.status === "DRAFT" ? "Nhập kho" : "Nhập thêm"}</Link></Button>}
