...
function ProductPrice({ price, salePrice, unit }: { price: number; salePrice: number | null; unit: string }) {
    ...
    if (!discounted) return <p className="mt-2 text-sm font-bold text-emerald-700">{price.toLocaleString("vi-VN")} đ/{unit}</p>;
    const percent = Math.round((1 - salePrice / price) * 100);
    return <div className="mt-2"><div className="flex flex-wrap items-center gap-1.5"><span className="text-xs text-slate-400 line-through">{price.toLocaleString("vi-VN")} đ</span><span className="rounded-md bg-red-100 px-1.5 py-0.5 text-[11px] font-black text-red-600">-{percent}%</span></div><p className="text-base font-black text-red-600">{salePrice.toLocaleString("vi-VN")} đ/{unit}</p></div>;
}
