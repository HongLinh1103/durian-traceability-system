...
export function TraceabilityManager({ initialLots, admin = false, readOnly = false, role = "FARMER", sources = [], destinations = [] }: { initialLots: Lot[]; admin?: boolean; readOnly?: boolean; role?: IssuerRole; sources?: SourceOption[]; destinations?: DestinationOption[] }) {
    ...
    const [saleMode, setSaleMode] = useState(role === "PROCESSING_FACILITY" ? "DOMESTIC" : "DIRECT");
    ...
    async function createLot(formData: FormData) {
        ...
        const payload = await response.json();
        if (payload.success) window.location.reload(); else setMessage(payload.error || "Không thể tạo lô bán / xuất hàng");
        setBusy(null);
    }
    ...
    return <div className="space-y-4">
        ...
        {!admin && !readOnly && <form action={createLot} className="space-y-5 rounded-3xl border bg-white p-5 shadow-sm">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <label className="text-sm font-bold">1. {role === "FARMER" ? "Lô thu hoạch" : role === "COLLECTOR" ? "Lô thu mua" : "Lô thành phẩm"}<select required name="sourceId" className="mt-1 w-full rounded-xl border px-3 py-2"><option value="">Chọn nguồn hàng</option>{sources.map(source => <option key={source.id} value={source.id}>{source.code} · {source.label}</option>)}</select></label>
                <label className="text-sm font-bold">2. Khối lượng<input required min="0.01" step="0.01" type="number" name="quantity" placeholder="Khối lượng kg" className="mt-1 w-full rounded-xl border px-3 py-2"/></label>
                <label className="text-sm font-bold">Tên sản phẩm<input required name="productName" placeholder="Tên sản phẩm" className="mt-1 w-full rounded-xl border px-3 py-2"/></label>
                <label className="text-sm font-bold">Mã lô bán / xuất hàng<input required name="lotCode" placeholder="Ví dụ: QR-20260824-001" className="mt-1 w-full rounded-xl border px-3 py-2"/></label>
                <label className="text-sm font-bold">3. Hình thức bán / xuất<select value={saleMode} onChange={event => setSaleMode(event.target.value)} className="mt-1 w-full rounded-xl border px-3 py-2">{role === "FARMER" ? <><option value="DIRECT">Bán trực tiếp</option><option value="RETAIL">Đưa đến điểm bán lẻ</option></> : role === "PROCESSING_FACILITY" ? <><option value="DOMESTIC">Bán trong nước</option><option value="EXPORT">Xuất khẩu</option></> : <option value="DOMESTIC">Xuất bán đến điểm bán lẻ</option>}</select></label>
                <label className="text-sm font-bold">Điểm đến có sẵn<select name="destinationId" className="mt-1 w-full rounded-xl border px-3 py-2"><option value="">Nhập điểm đến mới</option>{destinations.map(destination => <option key={destination.id} value={destination.id}>{destination.name}</option>)}</select></label>
            </div>
            <div className="border-t pt-4"><p className="mb-3 text-xs font-black uppercase text-slate-500">4. Khai báo điểm đến mới</p><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><select name="destinationType" value={saleMode === "EXPORT" ? "EXPORT" : undefined} defaultValue={saleMode === "EXPORT" ? undefined : "MARKET"} className="rounded-xl border px-3 py-2"><option value="MARKET">Chợ / chợ đầu mối</option><option value="RETAIL">Siêu thị / cửa hàng bán lẻ / gian hàng</option><option value="DISTRIBUTOR">Nhà phân phối</option><option value="EXPORT">Xuất khẩu</option><option value="OTHER">Khác</option></select><input name="destinationName" placeholder={saleMode === "EXPORT" ? "Đơn vị nhập khẩu" : "Tên điểm đến"} className="rounded-xl border px-3 py-2"/><input name="destinationAddress" placeholder={saleMode === "EXPORT" ? "Quốc gia, cảng đến, địa chỉ" : "Tỉnh/Thành, Quận/Huyện, Xã/Phường, địa chỉ"} className="rounded-xl border px-3 py-2"/><input name="plannedDate" type="date" title="Ngày dự kiến bán/giao/xuất" className="rounded-xl border px-3 py-2"/><input name="contactName" placeholder="Người nhận (không bắt buộc)" className="rounded-xl border px-3 py-2"/><input name="contactPhone" placeholder="SĐT liên hệ (không bắt buộc)" className="rounded-xl border px-3 py-2"/><textarea name="note" placeholder={saleMode === "EXPORT" ? "Cảng xuất, container, seal, tờ khai, chứng thư kiểm dịch (không bắt buộc)" : "Ghi chú (không bắt buộc)"} className="min-h-20 rounded-xl border px-3 py-2 sm:col-span-2 lg:col-span-3"/></div></div>
            <div className="flex items-center justify-between gap-3 rounded-2xl bg-emerald-50 p-4"><p className="text-sm font-semibold text-emerald-800"><b>5. Kiểm tra truy xuất</b><br/>Sau khi lưu lô bán, hệ thống hiển thị độ hoàn chỉnh và điều kiện phát hành QR.</p><Button disabled={busy === "create"}>{busy === "create" ? "Đang kiểm tra..." : "Kiểm tra dữ liệu"}</Button></div>
        </form>}
