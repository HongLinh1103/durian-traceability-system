...
export function TraceabilityManager({ initialLots, admin = false, readOnly = false, role = "FARMER", sources = [], destinations = [], initialSourceId = "" }: { initialLots: Lot[]; admin?: boolean; readOnly?: boolean; role?: IssuerRole; sources?: SourceOption[]; destinations?: DestinationOption[]; initialSourceId?: string }) {
    ...
    const nextSequence = Math.max(0, ...lots.map(lot => lot.lotCode.match(new RegExp(`^${prefix}-${dateCode}-(\\d+)$`))).map(match => Number(match?.[1] ?? 0))) + 1;
